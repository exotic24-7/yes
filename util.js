...
function sendRoomRequest(msg){
    console.log('sendRoomRequest called with', msg);
    if(msg !== undefined) {
        console.log('sending room request msg', msg);
        send(msg);
    }
    send({name: document.querySelector('.nickname').value});
    if (typeof window.menuInventory !== 'undefined' && window.menuInventory && typeof window.menuInventory.pack === 'function') {
        try { send({updatePetals: window.menuInventory.pack()}); } catch (e) { console.warn('menuInventory.pack failed', e); }
    }
    ...
    changeReady(false);
    if (typeof window.squadUI !== 'undefined' && window.squadUI && typeof window.squadUI.removeAllClients === 'function') {
        window.squadUI.removeAllClients();
    }
    if (typeof window.menuInventory !== 'undefined' && window.menuInventory && typeof window.menuInventory.sendQueuedChangedPetalsImmediately === 'function') {
        window.menuInventory.sendQueuedChangedPetalsImmediately();
    }
    if (typeof window.squadUI !== 'undefined' && window.squadUI && typeof window.squadUI.sendSavedStartingWave === 'function') {
        window.squadUI.sendSavedStartingWave();
    }
}
...
window.onmousedown = (e) => {
    ...
    if(typeof window.squadUI === 'undefined'){
        return;
    }

    if(window.squadUI.hoveringOverX === true && e.button === 0){
        closeSquadUI();
    }
    ...
    if(window.state === "menu" && e.button === 0 && window.squadUI && typeof window.squadUI.intersectingSliderBound === 'function' && window.squadUI.intersectingSliderBound({x: mouseX, y: mouseY})){
        if(typeof window.squadUI.startSliderDrag === 'function') window.squadUI.startSliderDrag(mouseX);
    }
    ...
    if(window.connected === true){
        if(performance.now() - window.lastRoomSentTime > 300){
            if(window.squadUI && window.squadUI.hoveringOverPublic === true && e.button === 0){
                sendRoomRequest({findPublic: true, biome: biomeManager.getCurrentBiomeData().current});
            } else if(window.squadUI && window.squadUI.hoveringOverNew === true && e.button === 0){
                sendRoomRequest({newSquad: true, biome: biomeManager.getCurrentBiomeData().current});
            } else if(window.squadUI && window.squadUI.hoveringOverPrivate === true && e.button === 0){
            ...
            } else if(window.squadUI && window.squadUI.hoveringOverJoinMainPvp === true && e.button === 0){
                sendRoomRequest({newSquad: true, biome: biomeManager.getCurrentBiomeData().current});
        ...
        } else if(typeof window.craftingMenu !== 'undefined' && window.craftingMenu && (window.craftingMenu.hoveringOverButton === true || window.craftingMenu.hoveringOverX) && e.button === 0){
        ...
        if (typeof window.menuInventory !== 'undefined' && window.menuInventory && typeof window.menuInventory.mouseDown === 'function') {
            window.menuInventory.mouseDown({mouseX, mouseY, menuInventory: window.menuInventory}, window.menuInventory);
        }
        ...
        if(globalInventory.menuActive === true && draggingPetalContainer === null){
            globalInventory.mouseDown({mouseX, mouseY}, (typeof window.menuInventory !== 'undefined' ? window.menuInventory : undefined));
        } else if(typeof window.craftingMenu !== 'undefined' && window.craftingMenu && window.craftingMenu.menuActive === true){
            if (typeof window.craftingMenu.mouseDown === 'function') window.craftingMenu.mouseDown({mouseX, mouseY}, e);
        }
    }
...
document.addEventListener("wheel", (e) => {
    ...
    const mouseX = mouse.x * canvas.w / window.innerWidth;
    const mouseY = mouse.y * canvas.h / window.innerHeight;
    if (typeof window.globalInventory !== 'undefined' && window.globalInventory && typeof window.globalInventory.updateScroll === 'function') window.globalInventory.updateScroll({x: e.deltaX, y: e.deltaY}, {mouseX, mouseY});
    if (typeof window.craftingMenu !== 'undefined' && window.craftingMenu && typeof window.craftingMenu.updateScroll === 'function') window.craftingMenu.updateScroll({x: e.deltaX, y: e.deltaY}, {mouseX, mouseY});
    if (typeof window.mobGallery !== 'undefined' && window.mobGallery && typeof window.mobGallery.updateScroll === 'function') window.mobGallery.updateScroll({x: e.deltaX, y: e.deltaY}, {mouseX, mouseY});
    if (typeof window.changelog !== 'undefined' && window.changelog && typeof window.changelog.updateScroll === 'function') window.changelog.updateScroll({x: e.deltaX, y: e.deltaY}, {mouseX, mouseY});
...
window.onmouseup = (e) => {
    ...
    if(window.state === "menu" && typeof window.squadUI !== 'undefined' && window.squadUI && window.squadUI.hoveringOverSlider === true && e.button === 0){
        if(typeof window.squadUI.endSliderDrag === 'function') window.squadUI.endSliderDrag(mouseX);
    }

    if(window.connected === true){
        // if(globalInventory.menuActive === true){
            globalInventory.mouseUp({mouseX, mouseY}, (typeof window.menuInventory !== 'undefined' ? window.menuInventory : undefined));
        // }
        ...
        if (typeof window.craftingMenu !== 'undefined' && window.craftingMenu && window.craftingMenu.menuActive === true){
            if (typeof window.craftingMenu.mouseUp === 'function') window.craftingMenu.mouseUp({mouseX, mouseY});
        }
        if(changelog.active === true){
...
function changeReady(state, toSend=true){
    if(toSend) {
        ...
        if(state === true && typeof window.squadUI !== 'undefined' && window.squadUI && Array.isArray(window.squadUI.clients) && window.squadUI.clients.filter(c => c.ready === false).length === 1){
            let _pack = {};
            if (typeof window.menuInventory !== 'undefined' && window.menuInventory && typeof window.menuInventory.pack === 'function'){
                try { _pack = window.menuInventory.pack(); } catch(e){ console.warn('menuInventory.pack failed', e); _pack = {}; }
            }
            send(Object.assign({changePetals: true}, _pack));
        }
        let _speed = undefined;
        if (typeof window.menuInventory !== 'undefined' && window.menuInventory && window.menuInventory.speedCircle && typeof window.menuInventory.speedCircle.reload !== 'undefined'){
            _speed = window.menuInventory.speedCircle.reload;
        }
        const readyMsg = {ready: state};
        if (typeof _speed !== 'undefined') readyMsg.updateSpeedCircle = _speed;
        send(readyMsg);
    }
    window.ready = state;
